...
While shock commands are received, the shock module delivers a short pulse of about 3 µs at about 3 kV (see Figure~\ref{shock}), about every 50 ms.
...
One should be able to adjust the shock intensity by setting the supply voltage for the high-voltage module. In the unmodified device, this is the minimum of the battery voltage and 3.3V. One should be able to adjust the shock intensity by providing an analog signal to R3. This would require a different µC (programmed to use the lower two data bits, and set the output to R3 accordingly, be it by integrated DAC, or some other form of DAC), either in addition to or replacing the exisitng µC. Replacing the µC would also get rid of the 3-minute auto-shutdown timeout at the same time.
