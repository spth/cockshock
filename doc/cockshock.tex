...
% Essentially, the Cock Shock is just a rebranded, repackaged dog shock collar with the collar band replaced by a shorter strip that better fits male genitals. TODO: Check this. There are devices that look just like that on ebay, but we'd want to know if the shock intensity is the same before statinge this here.
...
\begin{figure}
	...
	\caption{\label{remote-main}Scheamtic of remote (excluding sound module, RF module, light circuitry}
\end{figure}
...
\begin{figure}
	...
	\caption{\label{shockcommand}Top: Shock command as received from RF module at pin 7, bottom: pin 8 (total length of sequence: 50 ms, measured via LWLA1034 logic analyzer at shock unit)}
\end{figure}
...
If the pulse output for shocks from the µC can be used to set the shock intensity (e.g. by charging some capacitors just partially), a more advanced modification would be to replace the µC in the shock unit by a different one programmed to use the lower bits of the received data for the shock intensity (and also get rid of the 3-minute auto-shutdown timeout at the same time).
...
Modified remote controls can still be used with unmodified shock units and vice versa (of course then the shock volume will not be adjustable).
